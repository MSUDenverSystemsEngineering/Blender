...
Try {
	...
	[string]$appVendor = 'Blender'
	[string]$appName = 'Blender'
	[string]$appVersion = '2.83'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'blender,iexplore' -CheckDiskSpace -PersistPrompt
		...
		$exitCode = Execute-MSI -Path "$dirFiles\blender-2.83.0-windows64.msi" -Parameters "/qn" -PassThru
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-MSI -Action Uninstall -Path '{C39F5740-3CB7-44AC-B0A2-FA3B2754D02F}'
		Start-Sleep -s 10
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
